...
class AavailaPipeline:

    # Database setup
    conn = sqlite3.connect('availa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `availa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM availa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO availa (date, title, link, content)"
